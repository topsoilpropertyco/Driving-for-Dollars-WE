...
class FakeStatusD1 {
  prepare(sql) {
    ...
    return {
      bind: email => {
        assert.ok(["seth@example.test", "claire@example.test"].includes(email));
        return { first: async () => ({ active_devices: 1, account_active_devices: email === "seth@example.test" ? 1 : 0, points_received: 2, latest_received_at: "2026-09-20T12:00:02Z", latest_recorded_at: "2026-09-20T12:00:00Z" }) };
      },
    };
  }
}
...
const expected = { active_devices: 1, account_active_devices: 1, points_received: 2, latest_received_at: "2026-09-20T12:00:02Z", latest_recorded_at: "2026-09-20T12:00:00Z" };
...
assert.deepEqual(await shared.json(), { ...expected, account_active_devices: 0 });
