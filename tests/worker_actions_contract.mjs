...
const actions = [
  ...
  { event_id: "00000000-0000-4000-8000-000000000004", device_id: "synthetic-phone", sequence: 4, occurred_at: "2026-09-20T12:03:00Z", kind: "property_tagged", property_identity: propertyIdentity, payload: { condition: "needs_major_work", score: 4, latitude: 42.4, longitude: -82.9 } },
];
...
const claireActions = [
  { event_id: "00000000-0000-4000-8000-000000000005", device_id: "second-synthetic-phone", sequence: 1, occurred_at: "2026-09-20T12:04:00Z", kind: "property_saved", property_identity: claireProperty, payload: {} },
  { event_id: "00000000-0000-4000-8000-000000000006", device_id: "second-synthetic-phone", sequence: 2, occurred_at: "2026-09-20T12:05:00Z", kind: "property_tagged", property_identity: claireProperty, payload: { condition: "needs_minor_work", score: 6 } },
];
const claireAccepted = await worker.fetch(jsonRequest("/api/v1/actions", "claire@example.test", { actions: claireActions }), env);
assert.equal(claireAccepted.status, 202);
assert.deepEqual((await claireAccepted.json()).accepted_event_ids, claireActions.map(action => action.event_id));
...
assert.equal(body.summary.condition, "needs_major_work");
...
assert.deepEqual(await savedHomes.json(), { properties: [
  { property_identity: claireProperty, saved: true, stage: "no_outreach", condition: "needs_minor_work", score: 6, location: null, action_count: 2, last_activity_at: "2026-09-20T12:05:00Z" },
  { property_identity: propertyIdentity, saved: true, stage: "in_conversation", condition: "needs_major_work", score: 4, location: [-82.9, 42.4], action_count: 4, last_activity_at: "2026-09-20T12:03:00Z" },
] });
